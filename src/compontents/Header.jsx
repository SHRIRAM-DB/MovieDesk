...
function Header() {
  ...
  // It’s an active class. When you click on Home, Popular, Top-Rated, or Upcoming, it gets assigned a color
  ...
  // it's handle two condition one is dark mode and the another is change the image ex:(night to sun or sun to night)
  const [darkMode,setDarkMode] =  useState(localStorage.getItem("darkmode") || true);

  //i store as a value in local storage the stored value is false the darkmode is off but the stored value is true the darkmode will be on.
  //the useeffect is to set the dark mode and to remove the darkmode
  useEffect(() => {
...
<nav className="bg-white border-b-2 border-gray-200 dark:bg-gray-900">
  ...
  <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse">
      <img src={Logo} className="max-h-20 " alt="Flowbite Logo" />
      {/* <span className="self-center text-3xl font-semibold whitespace-nowrap dark:text-white">MovieDesk</span> */}
  </Link>
  <div id="mobile-nav" className="flex md:order-2">
    <div>
      <button onClick={() => setDarkMode(!darkMode)} data-collapse-toggle="navbar-search" type="button" className="mr-2 inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-search" aria-expanded="false">
