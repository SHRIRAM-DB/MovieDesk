...
function MovieDetails() {
  ...
  return (
    <main className='min-h-185'>
      <section className='flex justify-around flex-wrap py-19' >
        ...
        <div className='max-w-2xl text-gray-700 text-lg dark:text-white'>
          ...
          {/* if movie.length is greater then 0 show the genres else is a empty string*/}

          {movie.genres ? (
          <p className='my-7 flex flex-wrap gap-2'>
          {movie.genres.map((genre) => (
              <span key={genre.id} className="border border-gray-300 dark:border-gray-500 rounded p-2">{genre.name}</span>
          ))}
          </p>
          ...
          <div className="flex items-center">
              ...
              <span className=" text-gray-900 underline hover:no-underline dark:text-white">{movie.vote_count} reviews</span>
          </div>
